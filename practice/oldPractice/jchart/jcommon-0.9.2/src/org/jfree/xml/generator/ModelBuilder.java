 ...
 *
 * $Id: ModelBuilder.java,v 1.14 2004/03/05 13:46:41 mungady Exp $
 *
 * Changes
 * -------
...
public final class ModelBuilder {
    ...
    public DescriptionModel buildModel(SourceCollector c, DescriptionModel model) {
        ...
        Class[] baseClasses = findElementTypes(model);
    ...
    private Class[] findElementTypes(DescriptionModel model) {
        ...
        for (int i = 0; i < model.size(); i++) {
            ...
            for (int p = 0; p < properties.length; p++) {
                ...
                Class type = properties[p].getType();
    ...
    private void fillSuperClasses(DescriptionModel model) {
        ...
        for (int i = 0; i < model.size(); i++) {
            ...
            ClassDescription superCD = model.get(parent);
            if (superCD != null) {
    ...
    private Class[] fillModel(Class[] classes, DescriptionModel model) {
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            try {
                ...
                ClassDescription cd = createClassDescription(bi, parent);
                if (cd != null) {
...
//                        ("Ignore predefined property: ", propertyDescriptor.getName()));
        ...
        PropertyInfo[] propArray = (PropertyInfo[])
    ...
    public PropertyInfo createSimplePropertyInfo(PropertyDescriptor pd) {
        ...
        boolean readMethod = isValidMethod(pd.getReadMethod());
        boolean writeMethod = isValidMethod(pd.getWriteMethod());
    ...
    private String getHandlerClass(Class c) {
        if (BasicTypeSupport.isBasicDataType(c)) {
            String handler = BasicTypeSupport.getHandlerClass(c);
            if (handler != null) {
                return handler;
            }
        }
