 ...
 * Contributor(s):   -;
 *
 * $Id: TextBox.java,v 1.4 2004/03/25 13:45:07 mungady Exp $
 *
 * Changes
 * -------
...
public class TextBox implements Serializable {
    ...
    public void draw(Graphics2D g2, float x, float y, RectangleAnchor anchor) {
        Dimension2D d = this.textBlock.calculateDimensions(g2);
        double w = this.interiorGap.getAdjustedWidth(d.getWidth());
        double h = this.interiorGap.getAdjustedHeight(d.getHeight());
        d.setSize(w, h);
        Rectangle2D bounds = RectangleAnchor.createRectangle(d, x, y, anchor);
