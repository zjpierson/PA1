 ...
 * Contributor(s):   -;
 *
 * $Id: TextBlock.java,v 1.11 2004/03/24 13:28:53 mungady Exp $
 *
 * Changes
 * -------
...
public class TextBlock implements Serializable {
    ...
    public Dimension2D calculateDimensions(Graphics2D g2) {
        ...
        while (iterator.hasNext()) {
            ...
            Dimension2D dimension = line.calculateDimensions(g2);
            width = Math.max(width, dimension.getWidth());
            height = height + dimension.getHeight();
        }
        result.setSize((int) Math.ceil(width), (int) Math.ceil(height));
        return result;
    }
    ...
    public Shape calculateBounds(Graphics2D g2, 
                                 float anchorX, float anchorY, TextBlockAnchor anchor,
                                 float rotateX, float rotateY, double angle) {
        
        Dimension2D d = calculateDimensions(g2);
